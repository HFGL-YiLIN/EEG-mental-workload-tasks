function varargout = SVW1(varargin)
% SVW1 MATLAB code for SVW1.fig
%      SVW1, by itself, creates a new SVW1 or raises the existing
%      singleton*.
%
%      H = SVW1 returns the handle to a new SVW1 or the handle to
%      the existing singleton*.
%
%      SVW1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SVW1.M with the given input arguments.
%
%      SVW1('Property','Value',...) creates a new SVW1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SVW1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SVW1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SVW1
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @SVW1_OpeningFcn, ...
                   'gui_OutputFcn',  @SVW1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SVW1 is made visible.
function SVW1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SVW1 (see VARARGIN)

% Choose default command line output for SVW1
...
% UIWAIT makes SVW1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SVW1_OutputFcn(hObject, eventdata, handles) 
...
subjno = get(handles.subjnumber, 'String');
global outfile
outfile = fopen([datafolder 'SVW1_' num2str(subjno) '.csv'], 'w');
