function varargout = NBACK0(varargin)
% NBACK0 MATLAB code for NBACK0.fig
%      NBACK0, by itself, creates a new NBACK0 or raises the existing
%      singleton*.
%
%      H = NBACK0 returns the handle to a new NBACK0 or the handle to
%      the existing singleton*.
%
%      NBACK0('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NBACK0.M with the given input arguments.
%
%      NBACK0('Property','Value',...) creates a new NBACK0 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before NBACK0_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to NBACK0_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help NBACK0
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @NBACK0_OpeningFcn, ...
                   'gui_OutputFcn',  @NBACK0_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before NBACK0 is made visible.
function NBACK0_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to NBACK0 (see VARARGIN)

% Choose default command line output for NBACK0
...
correctAnsdb_noX = cell(trialNum_dbplus, cardNum - numel(correct_X)); % matrix(trialNum+nback X cardNum)
correctAnsdb_withX = cell(trialNum_dbplus, cardNum);
...
for i = 1:trialNum_dbplus
    ...
    for j=1:cardNum - numel(correct_X)
        correctAnsdb_noX{i,j} = cardColor{idx1(j)};
    end
...
for i = 1:cardNum
    for j = 1:trialNum_dbplus
        monitorAnsdb{i+7*(j-1)} = correctAnsdb{j,i};
    end
end
...
%        accuracy(trial_nth-nback) = 1;
       infoOncard = ['mind' monitorAnsdb{trial_nth-1-nback}];
       timeout = t2;
...
if isequal(infoOncard,correct_X{1})
...
% UIWAIT makes NBACK0 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = NBACK0_OutputFcn(hObject, eventdata, handles) 
...
subjno = get(handles.subjnumber, 'String');
global outfile
outfile = fopen([datafolder 'NBACK0_' num2str(subjno) '.csv'], 'w');
