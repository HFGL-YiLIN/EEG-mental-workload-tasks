function varargout = SVW2(varargin)
% SVW2 MATLAB code for SVW2.fig
%      SVW2, by itself, creates a new SVW2 or raises the existing
%      singleton*.
%
%      H = SVW2 returns the handle to a new SVW2 or the handle to
%      the existing singleton*.
%
%      SVW2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SVW2.M with the given input arguments.
%
%      SVW2('Property','Value',...) creates a new SVW2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SVW2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SVW2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SVW2
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @SVW2_OpeningFcn, ...
                   'gui_OutputFcn',  @SVW2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SVW2 is made visible.
function SVW2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SVW2 (see VARARGIN)

% Choose default command line output for SVW2
...
correctAnsdb = cell(trialNum+nback, cardNum); % matrix(trialNum+nback X cardNum)
...
fprintf(outfile, ',%d,%s,%2.4f\n', 1, probe_first, point2);
...
% UIWAIT makes SVW2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SVW2_OutputFcn(hObject, eventdata, handles) 
...
subjno = get(handles.subjnumber, 'String');
global outfile
outfile = fopen([datafolder 'SVW2_' num2str(subjno) '.csv'], 'w');
